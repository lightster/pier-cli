...
module Pier
  class WorkspaceConfig
    ...
    def has?(key)
      @overrides.has?(key) || @defaults.has?(key)
    end
    ...
    def project_dir(project)
      ...
      return repo_dir if project.include?('/') && Dir.exist?(repo_dir)
    ...
    def project_name_from_cwd(cwd)
      ...
      if org_name.empty? || project_name.empty?
    ...
    def load_from_workspace
      ...
      @codebase_dir = get('codebase_dir') if has?('codebase_dir')
